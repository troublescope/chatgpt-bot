...
class ChatGPTTelegramBot:
    ...
    def __init__(self, config: dict, openai: OpenAIHelper):
        ...
        self.commands = [
            BotCommand(
                command="help", description=localized_text("help_description", bot_language)
            ),
            BotCommand(
                command="reset", description=localized_text("reset_description", bot_language)
            ),
            BotCommand(
                command="image", description=localized_text("image_description", bot_language)
            ),
            BotCommand(
                command="stats", description=localized_text("stats_description", bot_language)
            ),
            BotCommand(
                command="resend", description=localized_text("resend_description", bot_language)
            ),
        ]
        self.group_commands = [
    ...
    async def transcribe(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async def _execute():
            ...
            try:
                ...
                # check audio duration and limit it to 1 minute
                if audio_track.duration_seconds > 60:
                    logging.warning(
                        f"Audio duration is {audio_track.duration_seconds} seconds, Transcription  canceled!"
                    )
                    audio_track = audio_track[:60000]
                    return
